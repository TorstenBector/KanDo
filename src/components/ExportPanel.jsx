...
async function buildExportText(selectedTagIds) {
  ...
  for (const item of items) {
    ...
    const attachmentCount = await db.item_images.where('item_id').equals(item.id).count()
    ...
    // Plain text can't usefully carry an image or file — a raw base64
    // string pasted into a chat isn't something an AI can actually look at
    // or open. Flag that attachments exist instead, and point at the actual
    // way to hand them over: a shared link (Dela-fliken), whose
    // get_shared_item_detail RPC returns the real data for anyone
    // (including an AI) to fetch.
    if (attachmentCount > 0) {
      lines.push(`📎 ${attachmentCount} bilag${attachmentCount === 1 ? 'a' : 'or'} bifogad${attachmentCount === 1 ? '' : 'e'} (ej med i textexport — dela en länk via "Dela" för åtkomst)`)
    }
    lines.push('')
  }
